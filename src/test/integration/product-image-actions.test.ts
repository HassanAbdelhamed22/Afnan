...
const mocks = vi.hoisted(() => ({ requireAdmin: vi.fn(), attach: vi.fn(), request: vi.fn(), approve: vi.fn(), remove: vi.fn(), order: vi.fn(), productCache: vi.fn(), categoryCache: vi.fn(), path: vi.fn() }));
...
describe("product image actions", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.requireAdmin.mockResolvedValue({ user: { id: "admin-1" } }); mocks.attach.mockResolvedValue(product); mocks.request.mockResolvedValue(product); });
