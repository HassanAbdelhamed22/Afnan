...
async function readSettings() { await connectMongoose(); const record = await StoreSettingsModel.findOne({ singletonKey: "STORE_SETTINGS" }).select("storeName adminEmail adminWhatsappE164 orderPrefix customRequestPrefix whatsappOrderTemplate socialLinks").lean(); return record ? map(record) : DEFAULT_STORE_SETTINGS; }
export async function getAdminStoreSettings() { return readSettings(); }
