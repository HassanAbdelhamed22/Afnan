...
async function result(operation: () => Promise<{ id: string; slug: string; categoryId: string }>, message: string): Promise<ActionResult<Data>> { try { const product = await operation(); refresh(product); return actionSuccess({ productId: product.id }, message); } catch (error) { if (error instanceof AppError) return actionFailure(error.code, error.message); return actionFailure("INTERNAL_ERROR", "Product image could not be updated"); } }

export async function attachProductImageAction(form: FormData): Promise<ActionResult<Data>> {
  const session = await requireAdmin();
  const parsed = attachProductImageSchema.safeParse({ productId: text(form, "productId"), intentId: text(form, "intentId"), alt: text(form, "alt") });
  if (!parsed.success) return actionFailure("VALIDATION_ERROR", "Add valid alt text before attaching the image");
  return result(() => attachProductImage(session.user.id, parsed.data.productId, parsed.data.intentId, parsed.data.alt), "Product image attached");
}
